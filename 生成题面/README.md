...
- `artifacts/batch_*.json`、`reports/batch_*.md`：目录批量生成时的整批汇总
...
当 `single` 模式省略 `--problem-ids` 时，系统会把 `--source-dir` 当前层级下的全部 `*.json` 文件视为一批任务，按文件名字典序逐个生成。批量模式要求每个文件显式提供 `problem_id`，并且该值必须与文件名一致。若中途任意一题在规划、生成或落盘阶段报错，整批会立即停止；此前已经成功落盘的结果会保留，同时额外写出 `batch_*.json` 与 `batch_*.md` 作为整批汇总。

运行过程中，控制台会输出当前阶段提示，包括参数校验、schema 归一化、进入流水线、当前题、当前 variant、规划、题面生成、产物写入与批量完成状态。
...
| `--prepared-schema-dir <归一化缓存目录>` | 归一化后的 schema 缓存目录 | 见 `config.py` | 全局 |
...
目录批量生成会额外产出 `batch_*.json` 与 `batch_*.md`。其中会记录：
