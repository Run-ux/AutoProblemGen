...
4. 按候选顺序规划，先执行代码级通用硬门槛；若仅出现变化轴声明不一致、规则必需轴未落地、差异度门槛或 helper 落地等规划合同错误，会用规划合同定向 prompt 最多额外修复 2 轮；通用硬门槛通过后再执行规则专属 LLM 规划审查，首选失败时会自动回退到下一候选
5. 把规划结果交给题面生成阶段，要求模型输出严格 JSON；若仅出现样例不足、输入/输出格式缺失、时间/空间限制缺失或计数题可数性声明缺失等合同错误，会用题面合同定向 prompt 最多额外修复 2 轮；通用结构校验通过后再执行规则专属 LLM 题面审查
...
### Schema Distance 计算方法

`Schema Distance `比较的是原始 schema 与候选 `new_schema` 在四元组上的结构差异。计算前会先把输入统一成标准四元组形态：`input_structure`、`core_constraints.constraints`、`objective`、`invariant.invariants`。距离值均裁剪到 `[0, 1]`，`0` 表示完全一致或近似一致，`1` 表示差异最大。
