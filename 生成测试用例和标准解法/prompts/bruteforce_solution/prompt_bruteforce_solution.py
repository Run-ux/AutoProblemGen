...
def build_system_prompt() -> str:
    return (
        ...
        "并设计朴素、直接、可验证正确的小规模真值 oracle。你不追求性能最优，只追求逻辑完整、边界正确、实现可靠。"
        "最终只输出单个 JSON 对象。"
    )
...
该解法的定位是“小规模真值 oracle”：用于在验证阶段为小规模合法输入产出可信真值。
它允许时间复杂度和空间复杂度很高，但必须保证：在小规模合法输入上，能够完整覆盖朴素枚举/搜索/模拟空间并得到正确输出。
不要求该解法在题面最大输入规模内可运行；最大规模过大、全空间无法穷举，不能单独作为返回 status="blocked" 的理由。""",
            """# 内部分析要求
请在内部完成以下检查，但不要输出冗长思维链：
1. 解析题意与输入结构，提取目标、格式、范围、输出要求、样例含义和特殊说明。
2. 使用最直接、最朴素的枚举、模拟、回溯、搜索或全排列等方法解决问题。
3. 根据题目结构自然选择适合暴力验证的小规模范围，不要为了覆盖题面最大规模而放弃生成暴力解。
4. 不要使用复杂优化、剪枝技巧或高阶数据结构，除非它们只是为了保证实现清晰。
5. 检查暴力算法在小规模输入上是否覆盖所有可能情况，没有遗漏边界条件。""",
            f"""# 代码要求
- 使用标准 Python。
...
- 不为样例硬编码。
- 若题目信息存在冲突、输入输出语义不足，或即使在小规模下也无法唯一确定正确答案，返回 status="blocked"，不要猜测生成代码。
- 不能仅因题面最大输入规模过大、最大规模下暴力复杂度不可接受或全规模空间无法穷举而返回 status="blocked"。""",
...
Part 4: 参考代码说明
Part 5: 复杂度分析
复杂度分析必须说明该暴力解适用于小规模验证；大规模输入可能超时或超内存，并会由验证流水线归类给标准解处理。
