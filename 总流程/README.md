...
默认情况下，未完成或失败的题会重新进入流水线，但四元组抽取阶段默认带 `--resume`：同一输出目录下 `status=success` 的 `tuple/raw/<problem_id>_<dimension>.json` 会被复用，失败、损坏或状态缺失的结果会重新抽取。若 `workflow_summary.json` 显示该题输入文件 hash 已变化，则四元组会重新抽取全部维度，避免旧四元组污染新输入。

如果运行命令带 `--skip-previous-failures`，历史已跑过但未 `verified` 且输入 hash 未变化的题会在本次运行中跳过，保留原状态，不会被标记为成功；未跑过的题仍会继续处理。因 `HTTP 402: Payment Required` 失败的题是例外：无论错误发生在四元组抽取、题面生成还是验证阶段，都会重新进入流水线。四元组抽取续传只复用 `status=success` 的维度，历史失败维度会重新请求并覆盖原结果。之后不带该参数运行，仍可按默认逻辑重跑其他失败题。输入 hash 变化的历史失败题不会被该参数跳过。
