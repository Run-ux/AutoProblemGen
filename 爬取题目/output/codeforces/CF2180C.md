...
Input
Each test contains multiple test cases. The first line contains the number of test cases $t$ ($1 \le t \le 10^4$). The description of the test cases follows. Each of the next $t$ lines contains two integers $n$ and $k$ ($1 \le n \le 10^9$, $1 \le k \le 10^5$). It is guaranteed that the sum of $k$ over all test cases does not exceed $10^5$.

Output
For each test case, output $k$ integers $a_1, a_2, \ldots, a_k$ such that $0 \le a_i \le n$. We can show that an answer always exists. If there are multiple valid answers, you may print any of them in any order.
