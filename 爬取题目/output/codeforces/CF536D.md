...
Input
The first line of input contains two integers n and m (2 ≤ n ≤ 2000, n - 1 ≤ m ≤ 10 5). The second line of input contains two integers s and t (1 ≤ s, t ≤ n, s ≠ t). The next line contains n integers p 1, p 2, ..., p n separated by spaces (| p i | ≤ 10 9). The next m lines contain the roads. Each line contains three integers v, u, w and it means that there's an road with length w between cities v and u (1 ≤ u, v ≤ n and 0 ≤ w ≤ 10 9). The road may lead from the city to itself, there may be several roads between each pair of cities.
