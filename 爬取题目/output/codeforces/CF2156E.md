...
The beauty of an array $b$ of length $m$ (with $m \ge 2$) is defined as the largest value of $b_j - b_i$ over all pairs of indices $i$ and $j$ such that $1\le i < j\le m$. More formally, it is equal to $\max\limits_{1\le i < j\le m} (b_j - b_i)$. Note that the beauty might be negative if the array is strictly decreasing.
