...
Input
The first line of input contains four integers, x, k, n and q (1 ≤ x ≤ k ≤ 8, k ≤ n ≤ 10 8, 0 ≤ q ≤ min (25, n - x)) — the number of pollywogs, the maximum length of jump, the number of stones and the number of special stones. The next line contains k integers, c 1, c 2, ... c k, separated by spaces (1 ≤ c i ≤ 10 9) — the energetic costs of jumps. The next q lines contain description of the special stones. Each line contains two integers p and w p (x + 1 ≤ p ≤ n, | w p | ≤ 10 9). All p are distinct.
