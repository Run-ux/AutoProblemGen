...
Input
Each test contains multiple test cases. The first line contains the number of test cases $t$ ($1 \le t \le 100$). The description of the test cases follows. The first and only line of each test case contains two integers $n$ and $m$ ($2\le n\le m\le 10^7$) — the parameters of the function. Note that there are no constraints on the sum of $n, m$ over all test cases.
