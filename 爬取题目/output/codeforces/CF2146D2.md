...
This is the hard version of the problem. The difference between the versions is that in this version, $0\leq l\leq r<2^{30}$. You can hack only if you solved all versions of this problem.
...
Input
Each test contains multiple test cases. The first line contains the number of test cases $t$ ($1 \le t \le 10^4$). The description of the test cases follows. The only line of each test case contains two integers $l$ and $r$ ($0\leq l \leq r<2^{30}$) — the minimum and maximum elements in $a$. Let $n = r - l + 1$. It is guaranteed that the sum of $n$ over all test cases does not exceed $2\cdot 10^5$.
