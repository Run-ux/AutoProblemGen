...
Input
The first line contains two integers — n and m (1 ≤ n ≤ 3·10 5, 2 ≤ m ≤ 1500). The next n lines contain four integers each, x 1, y 1, x 2, y 2 — the coordinates of the two endpoints of the segment. It's guaranteed that each segment has positive length. The next m lines contain three integers each, x i, y i, r i — the coordinates of the center and the radius of the i -th circle. All coordinates are integers of at most 10 4 in their absolute value. The radius is a positive integer of at most 10 4. It is guaranteed that all segments and all circles are dictinct.
