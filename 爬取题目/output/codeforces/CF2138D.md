...
Input
Each test contains multiple test cases. The first line contains the number of test cases $t$ ($1 \le t \le 10^3$). The description of the test cases follows. The first line of each test case contains three integers $n$, $m$, and $q$ ($1 \leq n,q \leq 5 \cdot 10^3$, $n \leq m \leq 10^9$) — the number of sliders, the length of the track, and the number of operations, respectively. The second line of each test case contains $n$ integers $a_1, a_2, \ldots, a_n$ ($1 \leq a_1 \lt a_2 \lt \ldots \lt a_n \leq m$) — the initial positions of each slider. Each of the next $q$ lines contains two integers $i$ and $x$ ($1 \le i \le n$, $i \leq x \leq m - n + i$) — the index of the slider to move, and the position to move the slider to, respectively. It is guaranteed that the sum of $n$ and the sum of $q$ over all test cases does not exceed $5\cdot 10^3$.
