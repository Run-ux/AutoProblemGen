...
Input
The first line contains two integers n, m (1 ≤ n, m ≤ 10 5, m > 1). The second line contains the first found number, the third line contains the second found number. The numbers are recorded as a sequence of digits in the m -based notation. Each digit is an integer from 0 to m - 1. The digits in the line are written in the order from the most significant digits to the least significant ones. The given numbers can contain leading zeroes.
