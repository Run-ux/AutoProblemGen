...
Input
The first line contains three space-separated integers n, m, k (1 ≤ n ≤ 10 5, 0 ≤ m ≤ 10 5, 1 ≤ k ≤ 10 9) — the number of clues, the number of direct clue links that Holmes has already found and the divisor for the modulo operation. Each of next m lines contains two integers a and b (1 ≤ a, b ≤ n, a ≠ b), that represent a direct link between clues. It is guaranteed that any two clues are linked by no more than one direct link. Note that the direct links between the clues are mutual.
